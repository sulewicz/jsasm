...
window.CodeMirror = (function () {
    ...
    function updateDisplayInner(cm, changes, viewPort) {
        ...
        var intact = changes === true ? [] : computeIntact([{ from: display.showingFrom, to: display.showingTo }], changes);
    ...
    function readInput(cm) {
        ...
        var from = sel.from,
            to = sel.to;
        if (same < prevInput.length) from = { line: from.line, ch: from.ch - (prevInput.length - same) };
        ...
        updateDoc(cm, from, to, splitLines(text.slice(same)), "end", cm.display.pasteIncoming ? "paste" : "input", { from: from, to: to });
    ...
    function updateDoc(cm, from, to, newText, selUpdate, origin) {
        ...
        if (split) {
            for (var i = split.length - 1; i >= 1; --i) updateDocInner(cm, split[i].from, split[i].to, [""], origin);
    ...
    function updateDocInner(cm, from, to, newText, selUpdate, origin) {
        ...
        if (view.history) addChange(cm, from.line, newText.length, old, origin, startSelFrom, startSelTo, view.sel.from, view.sel.to);
        return retval;
    }
    ...
    function unredoHelper(cm, type) {
        ...
        for (var i = set.events.length - 1; i >= 0; i -= 1) {
            ...
            updateDocNoUndo(cm, { line: change.start, ch: 0 }, { line: end - 1, ch: getLine(doc, end - 1).text.length }, change.old, selPos, type);
        }
    ...
    CodeMirror.prototype = {
        ...
        setLine: operation(null, function (line, text) {
            if (isLine(this.view.doc, line)) replaceRange(this, text, { line: line, ch: 0 }, { line: line, ch: getLine(this.view.doc, line).text.length });
        }),

        removeLine: operation(null, function (line) {
            if (isLine(this.view.doc, line)) replaceRange(this, "", { line: line, ch: 0 }, clipPos(this.view.doc, { line: line + 1, ch: 0 }));
        }),
    ...
    var commands = (CodeMirror.commands = {
        ...
        killLine: function (cm) {
            var from = cm.getCursor(true),
                ...
                sel = !posEq(from, to);
            if (!sel && cm.getLine(from.line).length == from.ch) cm.replaceRange("", from, { line: from.line + 1, ch: 0 }, "delete");
    ...
    function addChange(cm, start, added, old, origin, fromBefore, toBefore, fromAfter, toAfter) {
        ...
        if (
            cur &&
            (history.lastOp == cm.curOp.id || (history.lastOrigin == origin && (origin == "input" || origin == "delete") && history.lastTime > time - 600))
        ) {
